...
import { Operador, Ruta, Cliente, Entrega } from "../types";
...
export default function MiRutaPage() {
  ...
  const [rutaAsignada, setRutaAsignada] = useState<Ruta | null>(null);
  ...
  useEffect(() => {
    ...
    const idOp = datos.operadores.find((op: Operador) => op.nick === usuarioActivo.nick)?.id;
    if (idOp) {
      ...
      if (mapa) {
        ...
        if (rutaAsignada) {
          ...
          rutaAsignada.ordenClientes.forEach(idCli => {
            const cli = datos.clientes.find((c: Cliente) => c.id === idCli);
    ...
    <div>
        ...
        <div className="mb-6">
          ...
          <ol className="list-decimal list-inside mt-2 space-y-1">
            {rutaAsignada.ordenClientes.map(idCli => {
              ...
              const entrega = datos.entregas.slice().reverse().find((e: Entrega) => e.idCliente === idCli);
              return cli ? (
      ...
      {idClienteActivo && (
        <div className="p-4 bg-black/50 border border-red-800 rounded-lg mb-6">
          ...
          <select value={estadoEntrega} onChange={e => setEstadoEntrega(e.target.value as typeof estadoEntrega)}
