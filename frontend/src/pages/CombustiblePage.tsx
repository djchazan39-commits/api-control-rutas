...
export default function CombustiblePage() {
  ...
  const [form, setForm] = useState({ kmIni: 0, kmFin: 0, litros: 0, costo: 0 });
  ...
  async function guardar(e: React.FormEvent) {
    ...
    const nuevo: Combustible = {
      id: Date.now(), idOperador: idOp,
      kmIni: form.kmIni, kmFin: form.kmFin,
      litros: form.litros, costo: form.costo,
  ...
  return (
    <div>
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {datos.combustible.slice().reverse().map(c => (
