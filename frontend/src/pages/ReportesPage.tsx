  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-black to-red-950 text-white p-6">
          ...
          {Object.entries(ver).map(([clave, valor]) => (
            <label key={clave} className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={valor}
                onChange={e => setVer({ ...ver, [clave]: e.target.checked })}
        ...
        <div className="space-y-4">
          {ver.usuarios && (
            <div className="p-4 bg-white/5 rounded-lg">
              ...
              {datosApp?.usuarios?.map((u: any) => (
                <p key={u.id} className="text-sm text-gray-300">{u.nombre} — {u.rol}</p>
              ))}
            </div>
          )}
          ...
          {ver.operadores && (
            <div className="p-4 bg-white/5 rounded-lg">
              ...
              {datosApp?.operadores?.map((o: any) => (
                <p key={o.id} className="text-sm text-gray-300">
                  {o.nombre} — {o.licenciaClase || "Sin licencia"}
                </p>
              ))}
            </div>
          )}
          ...
          {ver.unidades && (
            <div className="p-4 bg-white/5 rounded-lg">
              ...
              {datosApp?.unidades?.map((u: any) => (
                <p key={u.id} className="text-sm text-gray-300">{u.placa} — {u.modelo}</p>
              ))}
            </div>
          )}
          ...
          {ver.clientes && (
            <div className="p-4 bg-white/5 rounded-lg">
              ...
              {datosApp?.clientes?.map((c: any) => (
                <p key={c.id} className="text-sm text-gray-300">{c.nombre}</p>
              ))}
            </div>
          )}
          ...
          {ver.rutas && (
            <div className="p-4 bg-white/5 rounded-lg">
              ...
              {datosApp?.rutas?.map((r: any) => (
                <p key={r.id} className="text-sm text-gray-300">
                  {r.nombre} — {r.zona || "Sin zona"}
                </p>
              ))}
            </div>
          )}
          ...
          {ver.combustible && (
            <div className="p-4 bg-white/5 rounded-lg">
              ...
              <p className="text-sm text-gray-300">
                Total Litros: {datosApp?.combustible?.reduce((s: number, r: any) => s + (Number(r.litros) || 0), 0).toFixed(2)} L
              </p>
              <p className="text-sm text-gray-300">
                Total Importe: ${datosApp?.combustible?.reduce((s: number, r: any) => s + (Number(r.importe) || 0), 0).toFixed(2)}
              </p>
            </div>
          )}
        </div>
      ...
      <div className="text-center mt-8 space-x-4">
        ...
        <button onClick={cerrarSesion} className="bg-red-800/70 hover:bg-red-700 px-6 py-3 rounded-lg font-bold">
          🚪 Cerrar Sesión
        </button>
      </div>
    </div>
