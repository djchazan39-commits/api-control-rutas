        ...
        {modo === "lista" ? (
                ...
                {lista.map((r: any) => {
                  ...
                  return (
                    <div key={r.id} className="p-3 bg-white/5 rounded-lg border border-white/10">
                      <p className="font-bold text-white">{r.nombre} <span className="text-sm font-normal text-amber-300">— {r.zona || "Sin zona"}</span></p>
                      <p className="text-sm text-gray-300">👷 {op?.nombre || "Sin operador"} | 🚛 {un?.placa || "Sin unidad"}</p>
                      <p className="text-sm text-gray-400">📍 {r.ordenClientes?.length || 0} clientes en recorrido</p>
                      <div className="mt-2 space-x-2">
                        <button onClick={() => editar(r)} className="text-yellow-400">✏️ Editar</button>
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                <label className="block text-sm text-gray-300 mb-1">Nombre de Ruta *</label>
                <input
              ...
              <div>
                <label className="block text-sm text-gray-300 mb-1">Operador *</label>
                <select
                  id="campo_op_ruta"
              ...
              {form.ordenClientes.length > 0 && (
                <div className="mb-3 p-3 bg-green-900/30 rounded-lg border border-green-500/30">
                  ...
                  {form.ordenClientes.map((idCliente, indice) => {
                    ...
                    return (
                      <div key={idCliente} className="flex items-center justify-between py-1 px-2 bg-white/5 rounded mb-1">
                        <span className="text-white text-sm">
                          <strong className="text-amber-400">{indice + 1}.</strong> {c?.nombre || "Cliente desconocido"}
                        </span>
                        <div className="flex gap-1">
                          <button onClick={() => moverCliente(idCliente, "arriba")} className="px-2 text-xs bg-gray-700 rounded text-white">↑</button>
                          <button onClick={() => moverCliente(idCliente, "abajo")} className="px-2 text-xs bg-gray-700 rounded text-white">↓</button>
                          <button onClick={() => toggleCliente(idCliente)} className="px-2 text-xs bg-red-700 rounded text-white">✕</button>
                        </div>
                      </div>
                    );
                  })}
                  ...
                  clientes.map((c: any) => {
                    ...
                    return (
                      <label key={c.id} className={`flex items-center gap-2 text-sm py-1 cursor-pointer ${yaSeleccionado ? "text-gray-400 line-through" : "text-white"}`}>
                        ...
                        {c.latitud && c.longitud && <span className="text-green-400 text-xs ml-2">✅ Con ubicación</span>}
                      </label>
                    );
                  })
                )}
            ...
            <div className="mb-4">
              ...
              <div
                id="mapa-ruta-previo"
                className="w-full h-72 rounded-lg border-2 border-amber-500/30"
            ...
            <div className="flex gap-3 mt-4">
              ...
              <button onClick={guardar} className="px-6 py-2 bg-green-600 hover:bg-green-700 rounded-lg text-white font-bold">💾 Guardar Ruta</button>
            </div>
          </>
        )}
      </div>
