    ...
    if (idOperadorSel === '') {
      clientes.forEach(c => {
        ...
        if (cli.latitud && cli.longitud) {
          const lat = parseFloat(cli.latitud);
          const lng = parseFloat(cli.longitud);
          if (!isNaN(lat) && !isNaN(lng)) {
            L.marker([lat, lng]).addTo(mapa).bindPopup(`<b>${cli.nombre}</b><br>${cli.direccion || ''}`);
          }
        }
      });
      return;
      ...
      ruta.ordenClientes?.forEach((idCli: number) => {
        ...
        if (c) {
          ...
          if (cli.latitud && cli.longitud) {
            ...
            if (!isNaN(lat) && !isNaN(lng)) {
              puntos.push([lat, lng]);
              L.marker([lat, lng]).addTo(mapa).bindPopup(`<b>${cli.nombre}</b><br>Orden: ${puntos.length}`);
            }
          }
        }
      });
